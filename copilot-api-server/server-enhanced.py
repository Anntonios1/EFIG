#!/usr/bin/env python3
"""
GitHub Copilot API Server - Enhanced with Tools & Vision
Compatible con OpenAI API y Ollama - Para n8n, Telegram bots, Open WebUI, etc.
"""
...
class GithubCopilotClient:
    """Cliente de GitHub Copilot API con soporte para Tools y Vision"""
    ...
    def _detect_request_features(self, messages: List[Dict]) -> Dict[str, bool]:
        """Detectar características del request: images, tools, etc."""
        features = {
            "has_images": False,
            "has_tools": False,
            "is_agent_call": False
        ...
        # Detectar imágenes (vision)
        for msg in messages:
            content = msg.get("content")
            
            # Formato lista (OpenAI)
            ...
            # Formato texto con URLs de imagen
            elif isinstance(content, str) and any(ext in content.lower() for ext in ['.jpg', '.jpeg', '.png', '.gif', '.webp']):
                features["has_images"] = True

        # Detectar tool calls o agent
        features["is_agent_call"] = any(
            msg.get("role") in ["assistant", "tool", "function"] 
            for msg in messages
        )
    ...
    def chat_completions_create(self, **kwargs) -> Iterator[bytes]:
        """Crear chat completion con soporte para tools y vision"""
        ...
        # Detectar características del request
        features = self._detect_request_features(messages)
        
        # Configurar headers según características
        headers["X-Initiator"] = "agent" if features["is_agent_call"] else "user"
        ...
        if tools:
            ...
            logger.info(f"🔧 Tools provided: {len(tools)}")
            # Copilot soporta tools nativamente, solo pasar el parámetro
            kwargs["tools"] = tools
            if "tool_choice" in kwargs:
                kwargs["tool_choice"] = kwargs["tool_choice"]
        ...
        logger.info(
            f"📡 Request → Model: {kwargs.get('model')}, "
            f"Messages: {len(messages)}, "
            f"Images: {features['has_images']}, "
            f"Tools: {features['has_tools']}, "
            f"X-Initiator: {headers['X-Initiator']}"
        )
        ...
        try:
            ...
            # Streaming de respuesta con buffer
            buffer = ""
            tool_calls_buffer = []
            ...
            while True:
                ...
                while "\n" in buffer:
                    ...
                    if line.startswith("data: "):
                        ...
                        if data_part == "[DONE]":
                            # Si hay tool_calls acumulados, enviarlos
                            if tool_calls_buffer:
                                logger.info(f"🔧 Tool calls detected: {len(tool_calls_buffer)}")
                        ...
                        try:
                            ...
                            # Detectar y acumular tool_calls
                            if "choices" in chunk_data:
                                for choice in chunk_data["choices"]:
                                    if "delta" in choice:
                                        delta = choice["delta"]
                                        
                                        # Tool call en el delta
                                        if "tool_calls" in delta:
                                            tool_calls_buffer.extend(delta["tool_calls"])
                                        
                                        # Function call (formato legacy)
                                        elif "function_call" in delta:
                                            function_call = delta["function_call"]
                                            # Convertir a formato tool_calls
                                            tool_call = {
                            ...
                            # Si no es JSON válido, pasar el chunk tal cual
                            yield line.encode("utf-8") + b"\n\n"
...
def index():
    """Página de inicio"""
    return jsonify({
        "service": "GitHub Copilot API Server Enhanced",
        "version": "2.0.0",
        "status": "running",
        "features": {
            "tools": True,
        ...
        "compatible_with": ["OpenAI API", "Ollama API", "Open WebUI"],
        "endpoints": {
            "models": "/v1/models",
...
def chat_completions():
    """Chat completions con soporte para tools y vision"""
    if copilot_client is None:
        return jsonify({"error": "Cliente no inicializado"}), 503
    ...
    try:
        ...
        logger.info(f"📨 Request recibido: model={data.get('model')}, messages={len(data.get('messages', []))}")
        ...
        if "tools" in data:
            ...
            logger.info(f"🔧 Tools incluidos: {len(data['tools'])}")
        ...
        if kwargs["stream"]:
            ...
            # Formato de respuesta compatible con OpenAI/Ollama
            response_data = {
            ...
            if tool_calls:
                ...
                logger.info(f"🔧 Tool calls en respuesta: {len(tool_calls)}")
...
if __name__ == "__main__":
    print("=" * 60)
    print("🚀 GitHub Copilot API Server Enhanced v2.0")
    print("=" * 60)
    print()
    print("✨ Características:")
    print("  🔧 Tool calling / Function calling")
    print("  🖼️  Vision support (imágenes)")
    print("  📡 Streaming optimizado")
    print("  🔄 Compatible con OpenAI API, Ollama y Open WebUI")
    print()
